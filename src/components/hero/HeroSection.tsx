...
const HeroSection = () => {
    ...
    const listenScrollEvent = () => {
        window.scrollY > 10 ? setNavBarTrans(true) : setNavBarTrans(false);
    };
    ...
    return (
        <motion.section className="w-full lg:h-[800px] md:h-[600px] h-[526px] flex flex-col md:rounded-[36px] rounded-[24px] items-center lg:justify-start bg-greenBg relative overflow-hidden heroGradient"
            variants={containerVariants}
            initial="hidden"
            animate="visible"
        >
            <Header navBarTrans={navBarTrans} />
            <motion.div className="flex flex-col items-center md:gap-2 gap-1 relative mt-10 lg:mt-44" initial='hidden' animate="visible" variants={zoomOutEffect}>
            ...
            <aside className="w-full md:px-[36px] flex md:flex-row flex-col md:justify-end md:items-end items-center gap-3 absolute bottom-[32px] left-0">

                <button className="flex flex-col items-center gap-1">
                    <span className="md:w-12 md:h-12 w-9 h-9 animate-bounce rounded-full border border-[#D1EFC1] flex justify-center items-center">
                        <HiOutlineArrowNarrowDown className="text-[#D1EFC1] md:scale-y-125" />
                    </span>
